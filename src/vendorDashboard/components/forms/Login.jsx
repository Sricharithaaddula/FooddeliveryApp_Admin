...
console.log("login API Response:", data); // 👈 Add this

if (response.ok) {
  // Success
} else {
  setError(data.message || "login Failed, Contact Admin");
  alert(data.message || "login Failed, Contact Admin");
}
          ...
          if (vendorId) {
            ...
            console.warn("Vendor ID not returned from login.");
          }
          
       } catch (error) {
        console.error("Login error:", error); // ⬅️ Add this
        alert("Login failed");
    }
     finally {
        setLoading(false); 
